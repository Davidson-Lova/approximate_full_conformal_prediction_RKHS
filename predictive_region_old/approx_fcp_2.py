...
class approx_fcp_2(cp):
    ...
    def _ncs_(self, X_train, Y_train, X_test, Z_test, params):
        ...
        for j in range(N_test):
            ...
            mus += [smallest_non_zero_eig(self.predictor.K / N_train_p1)]
    ...
    def predictor_qlty_bound(
        self, K_diag, mus, K_diag_3half_mean, d_loss_Np1_y, d_loss_Np1_z
    ):
        ...
        lam_Ns = [
            self.predictor.loss_bundle["lams"]["eta"] * mu / 2
        ...
        e_ = [
            min_loc(
                mul(
                    rho_2[j],
                    (K_diag[j][-1] ** 0.5) / ((lam_Ns[j] ** 3) * (N_train_p1**2)),
                ),
                mul(rho_1[j], 2 * (K_diag[j][-1] ** 0.5) / (lam_Ns[j] * N_train_p1)),
            )
            for j in range(N_test)
        ]
        return e_
                    ...
                    add(
                        ...
                        mul(
                            min_loc(
                                mul(
                                    rho_2[j],
                                    1 / ((lam_Ns[j] ** 3) * (N_train_p1**2)),
                                ),
                                mul(rho_1[j], 2 / (lam_Ns[j] * N_train_p1)),
                            ),
                            (K_diag[j][i] ** 0.5)
                            * (K_diag[j][-1] ** 0.5)
                            * self.predictor.loss_bundle["lams"]["beta"],
        ...
        if self.non_conformity_bundle["lams"]["reg"] == "C0":
            tau = [
                [
                    mul(
                        min_loc(
                            mul(rho_2[j], 1 / ((lam_Ns[j] ** 3) * (N_train_p1**2))),
                            mul(rho_2[j], 2 / (lam_Ns[j] * N_train_p1)),
                        ),
                        (K_diag[j][i] ** 0.5)
                        * (K_diag[j][-1] ** 0.5)
    ...
    def thickness_bound_explicit(self, K_diag, mus):
        ...
        facts = [
            ...
            for diag_max, lam in zip(diag_maxes, lams)
        ]

        ms = [
            0.5
            * self.predictor.loss_bundle["lams"]["xi"]
            * ((diag_max * fact * self.predictor.loss_bundle["lams"]["rho"]) ** 2)
            + (
                2
                * lam
                * diag_max
            ...
            for diag_max, lam, fact in zip(diag_maxes, lams, facts)
        ]

        bounds = [
            (12)
            * min(
                (diag_max * m) / ((lam**3) * (N_train_p1**2)),
                (2 * diag_max * self.predictor.loss_bundle["lams"]["rho"])
                / (lam * N_train_p1),
            )
            for diag_max, lam, m in zip(diag_maxes, lams, ms)
        ]
    ...
    def region(self, X_train, Y_train, X_test, Z_test, params):
        ...
        self.predictor.loss_params["dmax"] = max(y_max, y_min)
        self.predictor.update_loss(self.predictor.loss_params)
        ...
        tau = self.ncs_qlty_bound(
            K_diag, mus, d_loss_Np1_y, d_loss_Np1_z, d_ncs, K_diag_3half_mean
        )
